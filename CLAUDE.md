...
- **Path resolution**: every pipeline subdirectory is derived from `WORKSPACE_DIR`, set in `_lib.sh` (default `$HOME/Movies/OBS`, override via `export WORKSPACE_DIR=...`). Scripts use `SRC_DIR="$WORKSPACE_DIR/Recordings"` etc. Never reintroduce a hard-coded absolute path literal in a script — that breaks the new-machine and new-campaign setup paths documented in the README. The default lives in `_lib.sh`; don't duplicate it elsewhere.
...
2. Define `SRC_DIR` / `DST_DIR` as `"$WORKSPACE_DIR/Subdir"` (never as a hard-coded literal).
...
10. Decide whether the new stage is core (one of the always-run pipeline stages) or optional. Place it in `Scripts/pipeline/` or `Scripts/utils/` accordingly. Sourcing pattern for both: `source "$SCRIPT_DIR/../_lib.sh"`. Reference shared data files via `$SCRIPTS_DIR/names.txt` (set in `_lib.sh`) so the script doesn't depend on its own location.
...
$WORKSPACE_DIR/  README.md, CLAUDE.md, run.sh,
                 Recordings/, Audio/, Transcripts/, Summaries/, Scripts/
Scripts/         _lib.sh, names.txt, names.example.txt,
                 name_variants.txt, name_variants.example.txt
Scripts/pipeline/  extract_audio.sh, transcribe_audio.sh,
                 clean_transcript.sh, summarize_session.sh
Scripts/utils/   refine_summary.sh, audit_summaries.sh,
                 lint_glossary.sh, clear_session.sh
